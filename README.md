...
* スマホなどのデバイスの実機確認を必ず行う
  * レスポンシブの場合 `幅320px` でも破綻しないようにしてください
...
## [HTML](README_HTML.md)/[CSS](README_CSS.md)トピック
  ...
  * アイコンはなるべくWebIconFontを利用する（デザイナと相談）
...
* 顧客から指定されない場合の標準ターゲット（2020/10/08現在）
  * Windows環境
...
* ブレークポイント1つ2モードの場合(ほとんどの場合これ)
  * ブレークポイント
    * モバイル `834px` まで : PC `835px` から
  * 幅仕様
    * モバイル `可変幅100%` : PC `固定幅1100px`（1100px以下は横スクロールバー表示）
* ブレークポイント2つ3モードの場合
  * ブレークポイント
    * モバイル `414px` まで : タブレット `415px` から `834px` まで : PC `960px` から
  * 幅仕様
    * モバイル `可変幅100%` : タブレット `可変幅100%` : PC `固定幅1100px`
...
* 外部Webフォントを指定する場合、なるべく `google font` を利用するようにお願いしてください
  利用例
  ```
  https://fonts.googleapis.com/css?family=Noto+Sans+JP:400,500,700&display=swap
  ```
* 指示ならばしょうがないのですが、外部の日本語Webフォントはサイズが大きいのでなるべく使わないのがベターです
...
## 画像コンバートに関して
 * 幾何学的（写真ではない）画像でベクトル化できるものはベクトル画像にしてもらってください
 * 画像化を想定している文字も、フォントのままではなく、ベクトル化できるものはベクトル化してもらってください

## その他
 * フォントを指定する際はプロジェクト全体で使うフォントリストを頂いてください
